...
import { baseColumns, type TableColumnList } from '@/components/common/tag/columns'
import { getCommonTagList, removeCommonTag } from '@/api'
...
const delRowConfirm = async (record: any) => {
  const { error } = await removeCommonTag({ id: record.id })
  if (!error) {
    tableRef.value.reload()
  }
}
...
<template>
  <DashboardLayout>
    ...
    <DynamicTable
      ref="tableRef"
      :data-request="getCommonTagList"
      :columns="columns"
      scroll-x="min-w-[400px]"
    />
  </DashboardLayout>
  <CommonTagEdit

...
import { baseColumns, type TableColumnList } from '@/components/common/coins/columns'
import { getCommonCoinsList, removeCommonCcoins } from '@/api'
...
const delRowConfirm = async (record: any) => {
  const { error } = await removeCommonCcoins({ id: record.id })
  if (!error) {
    tableRef.value.reload()
  }
}
...
<template>
  <DashboardLayout>
    ...
    <DynamicTable
      ref="tableRef"
      :data-request="getCommonCoinsList"
      :columns="columns"

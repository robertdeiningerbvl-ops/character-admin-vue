...
import { baseColumns, type TableColumnList } from '@/components/common/message/columns'
import { getCommonMessageList, removeCommonMessage } from '@/api'
...
const openEditModal = async (record: any) => {
  ...
  if (record) {
    ...
    state.currentForm = { state: 2, ty: 2, image: '' }
  }
}
...
const delRowConfirm = async (record: any) => {
  const { error } = await removeCommonMessage({ id: record.id })
  if (!error) {
    tableRef.value.reload()
  }
}
...
<template>
  <DashboardLayout>
    ...
    <DynamicTable
      ref="tableRef"
      :data-request="getCommonMessageList"
      :columns="columns"
